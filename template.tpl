...
[
  {
    ...
    "subParams": [
      ...
      },
      {
        "type": "CHECKBOX",
        "name": "clearCookie",
        "checkboxText": "Clear cookie on conversion",
        "simpleValueType": true,
        "defaultValue": true
      },
      {
...
switch (eventModel.event_name) {
  case PAGE_VIEW_EVENT:
    ...
    if (parsedUrl && parsedUrl.searchParams) {
      for (let key in parsedUrl.searchParams) {
        if (key.toLowerCase() === 'c2bm') {
          const rawValue = parsedUrl.searchParams[key];

          if (typeof rawValue === 'string') {
            c2bmValue = rawValue;
          } else {
            c2bmValue = rawValue[0];
          }
    ...
    if (c2bmValue) {
      ...
      setCookie('_c2b_attribution_id', c2bmValue, options);
    }
  ...
  case GENERATE_LEAD_EVENT:
    const c2bCookie = getCookieValues('_c2b_attribution_id');

    if (!c2bCookie || !c2bCookie[0]) {
      data.gtmOnSuccess();
      break;
    }
    ...
    const url = 'https://analytics.clic2buy.com/api/events/from-tracking-param?' + urlParamsString;
        ...
        },
        {
          ...
          "value": {
            ...
            "listItem": [
              {
                "type": 1,
                "string": "https://analytics.clic2buy.com/*"
              }
            ]
          }
        }
...
scenarios:
- name: Test page view event with c2bm param
  code: |-
    ...
    const eventInput = {
      // Simule l'événement GTM entrant
      event_name: 'page_view',
      page_location: 'https://example.com/?c2bm=abc123'
    };

    // Mocks des "entrées"
    mock('getAllEventData', () => eventInput);
    mock('getCookieValues', () => []); // Simule l'absence de cookie existant

    // Mocks des "sorties"
    mock('setCookie', (name, value, options) => {
      // Valide le cookie qui sera créé
      assertThat(name).isEqualTo('_c2b_attribution_id');
      assertThat(value).isEqualTo('abc123'); // Valide qu'il a bien extrait la valeur
      assertThat(options['max-age']).isEqualTo(30 * 86400); // 30 jours
      assertThat(options.domain).isEqualTo('auto'); // L'équivalent SS de "isFirstParty"
    });
    mock('sendHttpRequest', () => {}); // Mock espion
...
- name: Test page view event without c2bm param
  code: "// 1. ARRANGE: Définir le contexte et les mocks\nconst mockData = {\n  //\
    \ Champs de l'UI pour runCode(data)\n  pageViewEvent: 'page_view',\n  cookieExpiration:\
    \ 30\n};\n\nconst eventInput = {\n  // Simule l'événement GTM entrant\n  event_name:\
    \ 'page_view',\n  page_location: 'https://example.com/une-page-classique?autre_param=123'\n\
    };\n\n// Mocks des \"entrées\"\nmock('getAllEventData', () => eventInput);\nmock('getCookieValues',\
    \ () => []); // Simule l'absence de cookie existant\n\n// Mocks des \"sorties\"\
    \ (on les prépare en \"espions\")\nmock('setCookie', () => {}); \nmock('sendHttpRequest',\
    \ () => {});\n\n// 2. ACT: Exécuter le code\nrunCode(mockData);\n\n// 3. ASSERT:\
    \ Vérifier le comportement\n// VÉRIFICATIONS CLÉS : Le code ne doit RIEN faire\n\
...
Created on 13/11/2025 16:18:06
